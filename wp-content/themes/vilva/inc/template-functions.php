...
/**
 * Add custom title2 to post
 */
function additional_title_for_post() {
    global $post;

    if (get_field('title2', $post->ID)) : ?>

        <h5><?php the_field('title2', $post->ID); ?></h5>

    <?php endif;
}
add_action( 'vilva_post_entry_content', 'additional_title_for_post', 11, 3 );
add_action( 'vilva_before_page_entry_content', 'additional_title_for_post', 11, 3 );

/**
 * Add custom description2 to post
 */
function additional_description_for_post() {
    global $post;

    if (get_field('description2', $post->ID)) : ?>

        <p><?php the_field('description2', $post->ID); ?></p>

    <?php endif;
}
add_action( 'vilva_post_entry_content', 'additional_description_for_post', 30, 3 );
